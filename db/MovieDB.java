...
public class MovieDB {
    ...
    private static final Path moviesPath = Paths.get("movies.txt");
    private static final File moviesFile = moviesPath.toFile();
    ...
    public static ArrayList<Movie> getMovies(String category) {
        ...
        //To push results to a new file.
//        try (PrintWriter out = new PrintWriter( new BufferedWriter( new FileWriter(moviesFile)))) {
        	for(Movie individualMovie : allMovies) {
       
        		if (individualMovie.getCategory().equalsIgnoreCase(category)) {
        	        movies.add(individualMovie); 
        	        System.out.println(individualMovie.getTitle());
        	        
        	        //Only if I want to push items to a new file
//        	        out.print(individualMovie.getTitle() + "\n");
        		}
